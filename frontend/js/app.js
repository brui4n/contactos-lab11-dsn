const API_URL = 'http://localhost:5001/api/contactos';
...
async function cargarContactos() {
    try {
        ...
        contactosList.innerHTML = '<tr><td colspan="5" class="text-center text-danger">Error de conexión</td></tr>';
    }
}

async function guardarContacto(data) {
    const isEdit = !!data.id;
    const url = isEdit ? `${API_URL}/${data.id}` : API_URL;
...
form.addEventListener('submit', (e) => {
    ...
    const data = {
        id: document.getElementById('contactoId').value || undefined,
